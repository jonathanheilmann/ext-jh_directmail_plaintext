 ...
 *
 */
class tx_jhdirectmailplaintext_pi1 extends AbstractPlugin {
	/**
	 * @var tslib_cObj
	 */
	var $cObj;
	var $conf = array();
	var $prefixId = 'tx_jhdirectmailplaintext_pi1';
	var $scriptRelPath = 'pi1/class.tx_jhdirectmailplaintext_pi1.php';
	var $extKey = 'jh_directmail_plaintext';
	var $charWidth = 76;
	var $linebreak;
	var $siteUrl;
	...
	function main($content,$conf) {
		...
		$lines = array();
		$skipDefaultCTypeRendering = FALSE;

		// Hook for pre-processing other content types
		if (is_array($TYPO3_CONF_VARS['EXTCONF']['jh_directmail_plaintext']['preRenderCType'])) {
			foreach($TYPO3_CONF_VARS['EXTCONF']['jh_directmail_plaintext']['preRenderCType'] as $_funcRef) {
				if ($_funcRef) {
					$_funcRef = GeneralUtility::callUserFunction($_funcRef, $lines, $skipDefaultCTypeRendering, $this);
				}
			}
		}

		$CType = (string)$this->cObj->data['CType'];
		if ($skipDefaultCTypeRendering === FALSE && GeneralUtility::inList(strtolower($this->conf['disableCTypeRendering']), $CType) !== TRUE) {
			switch($CType) {
				case 'header':
				...
				case 'textpic':
					...
					if ($header != '') $lines[] = $header;
					//$lines[] = $this->getHeader();
					if (($CType == 'textpic') && !$this->conf['disableImageRendering'] && !($this->cObj->data['imageorient']&24))	{
				...
				default:
					...
					if (is_array($TYPO3_CONF_VARS['EXTCONF']['jh_directmail_plaintext']['renderCType'])) {
						foreach($TYPO3_CONF_VARS['EXTCONF']['jh_directmail_plaintext']['renderCType'] as $_classRef) {
		...
		// Hook for post-processing other content types
		if (is_array($TYPO3_CONF_VARS['EXTCONF']['jh_directmail_plaintext']['postRenderCType'])) {
			foreach($TYPO3_CONF_VARS['EXTCONF']['jh_directmail_plaintext']['postRenderCType'] as $_funcRef) {
				if ($_funcRef) {
					$_funcRef = GeneralUtility::callUserFunction($_funcRef, $lines, $this);
				}
			}
		}
	 ...
	 * Uses FAL to support TYPO3 CMS 6.2 LTS and newer
	 *
	 * @return	string		Content
	 */
	function getImages() {
		...
		foreach ($fileObjects as $key => $value) {
            /** @var \TYPO3\CMS\Core\Resource\File $value */
			$this->cObj->setCurrentFile($value);
			$imageConf = $GLOBALS['TSFE']->tmpl->setup['tt_content.']['image.']['20.']['1.'];
			$imageConf['file.']['treatIdAsReference'] = 1;
			$imageConf['file'] = $value->getUid();
			$imgPath = $this->siteUrl . $this->cObj->IMG_RESOURCE($imageConf);
			$currentFileProperties = $this->cObj->getCurrentFile()->getProperties();
			
			// Render image
			$images_arr[] = array(
					'path' => $imgPath,
					'link' => !$this->cObj->data['image_zoom']?$currentFileProperties['link']:'',
					'caption' => $currentFileProperties['description']
			);
			
		}
		$images = $this->renderImages($images_arr);
	...
	function parseBody($str, $altConf='bodytext') {
		...
		$aConf['parseFunc.']['tags.']['a.']['userFunc'] = 'tx_jhdirectmailplaintext_pi1->atag_to_http';
		$aConf['parseFunc.']['tags.']['a.']['siteUrl'] = $this->siteUrl;
		$aConf['parseFunc.']['tags.']['a.']['link.']['stdWrap.'] = $this->conf[$altConf.'.']['link.']['stdWrap.'];
	...
	/**
	 * Render images - which means creating lines with links to the images.
	 *
	 * @param	array		$images: array of images with data
	 * @return	string		Content
	 * @see getImages()
	 */
	function renderImages($images) {
		...
		foreach ($images as $key => $image) {
			...
			if ($image['caption']) {
				...
				$lines[] = $this->breakContent($image['caption']);
			}
		}
	...
	function atag_to_http($content,$conf) {
		...
		return $this->cObj->getCurrentVal() . $this->cObj->stdWrap($theLink, $this->conf['link.']['stdWrap.']);
	}
